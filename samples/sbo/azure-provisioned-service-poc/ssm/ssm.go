...
func getPSQLParams(baseDir string) map[string]string {
	...
	for _, dir := range dirs {
		...
		if btype == "postgresql" && provider == "azure" {
...
func getMasterKey(baseKey string) string {
	...
	connParams := getPSQLParams("/home/baiju/Azure/bindings")
	connStr := fmt.Sprintf("host=%s user=%s password=%s dbname=%s sslmode=require port=%s", connParams["host"],
